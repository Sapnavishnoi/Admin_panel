...
import { getTagline } from "../../services/roleService";
...
const Tagline = () => {
  ...
  React.useEffect(() => {
    async function fetchData() {
      try {
        ...
        showSnackbar("Home Tagline detail fetched", "success");
        setLoading(false);
      } catch (e) {
        showSnackbar(e.message || "Unable fetch");
        setLoading(false);
      }
    }
  ...
  return (
    <ContentShell
      title="Home Tagline Detail"
      actions={
        <>
          <SearchField value={searchValue} handleChange={optimizedSearch} />
          <ExportButton list={filteredlist} fileName="home_report" />
        </>
      }
      loading={loading}
    >
